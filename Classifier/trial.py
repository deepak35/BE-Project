...
class NLTKPreprocessor(BaseEstimator, TransformerMixin):
	...
	def tokenize(self, document):
		...
		for sent in sent_tokenize(document):
			#Break the sentences into part of speech tagged tokens
...
@timeit
def build_and_evaluate(X, y, classifier=MultinomialNB, outpath=None, verbose=True):
	"""
	...
	def build(classifier, X, y=None):
		""" Inner function that builds a single model"""
	...
	if verbose: print("Building for evaluation")
    	...
    	if outpath:
    		...
    		print ("Model written to {}".format(outpath))
...
def show_most_informative_features(model, text=None, n=20):
	"""
	Accepts a Pipeline with a classifier and a TfidfVectorizer and computes
	the n most informative features of the model. If text is given, then will
	compute the most informative features for classifying that text.
	Note that this function will only work on linear models with coefs_
	"""

	#Extract the vectorizer and classifier from the pipelie
	...
	if not hasattr(classifier, 'coef_'):
		raise TypeError(
			"Cannot compute informative features on {} model.".format(
				classifier.__class__.__name__
			)
		)

	if text is not None:
		#Compute coefficients for text
		tvec = model.transform([text]).toarray()

	else :
		#Otherwise simply use coefficients
